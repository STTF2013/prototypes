...
void setup() {
  ...
  matrices[1].begin(0x72); // pass in the address
  matrices[2].begin(0x71); // pass in the address
...
int readX() {
  return pixels*width - map(analogRead(A0),0,1023,0,pixels*width);
}

int readY() {
  return 7-map(analogRead(A1),0,1023,0,pixels*height-1)-9;
}
